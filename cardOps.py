...
ANSWER = 9# <--USER ENTERS DESIRED CARD THEY ARE TRYING TO GET TO HERE 
MYCARD_NAMES = ['4', '4', '8', 'K', 'Q'] # <--USER ENTERS CARDS IN HAND HERE
# use subsets as in
...
def main():
    ...
    for i in range(len(MYCARDS)):
        SUBSET = MYCARDS[0:i+1]
        path = graph(0, [], SUBSET, True) # operator paths to get to answer 
        if path:
            break
